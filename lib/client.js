 ...
 *
 * Visible surfaces, so the plugin's operations are reachable from the UI rather
 * than only from its HTTP routes:
 *   - one row in a Session's "..." menu (the delete entry),
 *   - a persistent sidebar-foot button opening the session picker,
 ...
 *     plus the same body in this plugin's own overlay, because the shell gives a
 *     third-party plugin no way to open its settings panel.
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      .dshsd-manageBody{max-height:70vh;overflow-y:auto}
    ...
    const zh = {
      ...
      "dialog.doneBody": "会话目录已移到回收站，内容仍在磁盘上。可以在「管理回收站」里恢复或彻底删除。",
    ...
    const en = {
      ...
      "dialog.doneBody": "The session directory now sits in the recycle bin; its bytes are still on disk. Use Manage recycle bin to restore or permanently delete it.",
    ...
    /**
     * The client context, captured in `apply` so callbacks defined out here can
     * reach its services. `ctx.layout` is a service instance, not a service
     * container: use `ctxRef.services.layout.selectPanel(...)`.
     */
     ...
     *
     * The shell owns the settings panel and exposes no client service to open
     * it: `ctx.layout.selectPanel(id)` only addresses a main panel registered in
     * the `sidebar.panellist` slot (whose shipped occupants are `plugins` and
     * `dsh-market`), and selecting an unregistered key throws. The settings
     * panel is a shell-owned modal whose open state lives inside
     * ui-settings-general's own component, reachable only through the sidebar
     * foot trigger. So this is a best-effort attempt, and the caller must have a
     * working fallback rather than depending on it.
     *
     * Returns true when the shell accepted the navigation.
     */
    function trySelectSettingsPanel() {
      try {
        const layout = ctxRef.services && ctxRef.services.layout;
        if (layout && typeof layout.selectPanel === "function") {
          layout.selectPanel("settings");
          return true;
        }
      } catch {
        // An unregistered panel key throws. That is expected on this build and
        // is why every caller opens the in-plugin overlay instead.
      }
      return false;
    }

    /**
     * Open the management view from an in-plugin entry point.
     *
     * The shell's settings panel is not reachable from a third-party plugin
     * (see trySelectSettingsPanel), so this always opens the plugin's own
     * overlay. The shell navigation is attempted first and only as a courtesy:
     * on a build where "settings" happens to be a registered main panel, the
     * reader lands in the real settings page, which contains the same body.
     *
     * Order matters — the overlay is opened either way, because a failed
     * `selectPanel` silently does nothing and a no-op button is the exact bug
     * this replaces.
     */
    function openManagePage() {
      trySelectSettingsPanel();
      openManage();
    }
      ...
      const footer = phase === "done"
        ? [
            j(primitives.Button, {
              key: "settings",
              variant: "outline",
              onClick: () => { closeDeleteDialog(); openManagePage(); },
     ...
     * than only from HTTP.
     *
     * Rendered twice from one body: as the inline `settings.section` occupant for
     * readers who open the shell's settings panel, and inside the plugin's own
     * overlay (below) for every in-plugin entry point. The shell exposes no
     * client service that opens its settings panel, so the entries that are meant
     * to lead there open the overlay instead; one body keeps the two paths from
     * drifting apart.
     */
    function SessionDeleterSection(props) {
      return j(SessionDeleterPanel, { t: props.t });
    }

    /** The manage overlay, used by entries that cannot reach the settings panel. */
    const manageStore = createStore({ open: false });
    function openManage() { manageStore.set({ open: true }); }
    function closeManage() { manageStore.set({ open: false }); }

    function SessionDeleterManageOverlay(props) {
      const t = props.t;
      const state = react.useSyncExternalStore(manageStore.subscribe, manageStore.get);
      if (!state.open) return null;
      return j(primitives.Modal, {
        open: true,
        onClose: closeManage,
        ...
        closeLabel: t("dialog.close"),
        contentClassName: "dshsd-manageBody",
        footer: j("div", {
          className: "dshsd-foot",
          children: j(primitives.Button, {
            variant: "primary",
            onClick: closeManage,
            children: t("dialog.close"),
          }),
        }),
        children: j(SessionDeleterPanel, { t }),
      });
    }

    function SessionDeleterPanel(props) {
      const t = props.t;
        ...
        footer: j("div", {
          ...
          children: [
            j(primitives.Button, {
              key: "manage",
              variant: "outline",
              onClick: () => { closePicker(); openManagePage(); },
    ...
    function apply(ctx) {
      ...
      // The management view as an overlay. The shell's settings panel cannot be
      // opened from a third-party plugin, so every in-plugin entry that promises
      // "manage" lands here instead of silently doing nothing.
      ctx.slots.inject("shell.overlay", () => ctx.slots.register({
        name: "shell.overlay",
        id: "session-deleter.manage",
        order: 5,
        locale: NS,
      }, SessionDeleterManageOverlay));
    }
    //#endregion
